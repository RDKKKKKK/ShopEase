  ...
  <div class="header">
    ...
    <div class="right">
      <el-popover placement="bottom" :width="320" trigger="click" popper-class="popper-user-box">
        ...
        <div class="nickname">
          <p>Name：{{ state.userInfo && state.userInfo.loginUserName || '' }}</p>
          <p>NickName：{{ state.userInfo && state.userInfo.nickName || '' }}</p>
...
const getUserInfo = async () => {
  ...
  const userInfo = await axios.get(`/users/admin/${token}`)
  state.userInfo = userInfo
}
// 退出登录
const logout = () => {
  axios.delete('/users/admin/logout').then(() => {
